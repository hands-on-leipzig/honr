...
## Icons and storage (badges, program/season/role logos)

- **Role icons** (badges per role on /awards): In repo at `backend/public/images/logos/roles/`. Generated by `php artisan roles:generate-icons --generate-svgs` (run on deploy); uploads via Admin UI also go here. Commit the generated/uploaded files so they deploy.
- **Program and season logos**: In repo at `backend/public/images/logos/programs/` and `backend/public/images/logos/seasons/`. Default SVGs are committed. **To have custom logos on test (and production) after every deploy, add the image files to these directories and commit them to GitHub.** Filenames: `{id}.jpg` (or `.png`, `.svg`, `.webp`) — e.g. `1.jpg` for program ID 1. See `backend/public/images/logos/README.md`. Logos uploaded only via Admin UI on the server are not in the repo and will be missing after a fresh deploy; copy the file into the repo and commit if you want it to persist.
- **Moving icons from storage into the repo**: If you had icons under `backend/public/storage/logos` (gitignored), run `bash scripts/move-logos-from-storage-to-repo.sh` from the repo root, then run migrations and commit the files under `backend/public/images/logos/`.
- **Storage link**: Deployment runs `php artisan storage:link` so `public/storage` points to `storage/app/public` (used for other uploads if any). The web server must serve `/storage` from that path (see WEB_SERVER_CONFIGURATION.md).
