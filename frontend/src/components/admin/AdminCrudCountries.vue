<template>
    ...
    <div class="p-4 border-b border-gray-200">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          <h2 class="text-xl font-semibold">Länder</h2>
        ...
        <div class="flex items-center space-x-3">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-1 mt-1 text-xs">
              <span class="px-2 py-0.5 bg-gray-100 rounded-full">{{ item.locations_count }} Standorte</span>
  ...
  <div v-if="editingItem" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-md">
      <div class="p-4 border-b border-gray-200 flex items-center justify-between">
        <h3 class="text-lg font-semibold">{{ editingItem.id ? 'Land bearbeiten' : 'Neues Land' }}</h3>
      ...
      <form @submit.prevent="saveItem" class="p-4 space-y-4">
        ...
        <div>
          ...
          <input v-model="form.iso_code" type="text" required maxlength="2" class="w-full px-3 py-2 border border-gray-300 rounded-md uppercase" placeholder="z.B. DE" />
          <p class="text-xs text-gray-500 mt-1">2-stelliger ISO 3166-1 Code</p>
        </div>
        <div>
  ...
  <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg w-full max-w-sm p-6">
      <h3 class="text-lg font-semibold mb-4 text-red-600">Land löschen?</h3>
      <p class="mb-4 text-gray-600">
        Möchtest du das Land <strong>{{ editingItem?.name }}</strong> wirklich löschen?
      </p>
...
const API_PATH = '/admin/countries'
...
const filteredItems = computed(() => {
  ...
  if (searchQuery.value.trim()) {
    ...
    result = result.filter(i =>
      ...
      i.iso_code?.toLowerCase().includes(q)
    )
  }
  return result
})
